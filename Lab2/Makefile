...
# grading server
GRADING_SERVER?=http://sp01.snucse.org:8088
LAB:=lab2
...
test:
	@echo ">>> Submitting $(LAB) to grading server..."
	@tar -czf /tmp/$(LAB)_submit.tar.gz Makefile $(SRC_DIR)/
	@curl -s -X POST $(GRADING_SERVER)/submit \
		-F "lab=$(LAB)" \
