...
#
# Before run this script run your server with the same port number and delay of 5 seconds
# e.g., ./server -p 8080 -d 5
#
...
while getopts "p:" opt; do
    case $opt in
        p) PORT=$OPTARG ;;
        *) echo "Usage: $0 [-p port] [1|2|3|4|5]"; exit 1 ;;
    esac
done
...
case $TEST_SET in
    ...
    5)
        ...
        ;;
    *)
        echo "Invalid test set number. Use 1 ~ 5."
        exit 1
        ;;
esac
...
for i in "${!REQUESTS[@]}"; do
    echo "Sending Request $i: '${REQUESTS[$i]}'"
    {
        ...
        echo "${REQUESTS[$i]}" | stdbuf -oL ./client -p $PORT | ts '[%Y-%m-%d %H:%M:%S]' > "$OUTPUT_DIR/response_$i.log"
